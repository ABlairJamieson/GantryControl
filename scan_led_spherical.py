...
def main():
    ...
    gantry_to_target_offset_mm = 245#140 #218
    ...
    for n,gset in enumerate(gsets):
        ...
        # wait time at each gset
        time.sleep(wait_time)


        image_capture_time = (datetime.datetime.now()-DELAY_TIME).strftime("%H:%M:%S")
        if n==0:
            print("0 PITCH")
            #gantry.move( "DM", "DM","DM","DM",0,1000,1000,1000,200,200)
            time.sleep(TIMELAPSE_INTERVAL)
            image_capture_time = datetime.datetime.now().strftime("%H:%M:%S")
            time.sleep(DELAY)
            
        print(n, " image taken at ", image_capture_time, "theta = ", curtheta, " phi = ", curphi )  
        start_time = time.time()
        savefile = "pdScan2_capture_points.txt"
        with open(savefile, 'a') as file:
            file.write(f"{n},{image_capture_time},{curtheta},{curphi}\n")
        if n!=0 and time_elapsed > 30:
            print("not enough time to move the gantry, increase timlapse interval")
            
        #_ = input(f"Reached setpoint {n}, press ENTER to continue.")
    print('Done')
